...
import { fetchAdminConversations, PAGE_SIZE, type ConversationSummary } from "@/lib/admin";
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-xl font-bold text-foreground">Conversations</h1>
        ...
        <div className="space-y-2">
          {rows.map((c) => (
            <div key={c.id} className="rounded-xl border border-border bg-surface p-4">
              ...
              <TechnicalDetails items={[{ label: "Conversation UUID", value: c.id }]} />
            </div>
          ))}
        </div>
      )}
